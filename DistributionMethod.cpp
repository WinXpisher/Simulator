    ...
    Resource* outRes) const
{
    ...
    // Обробка завдання за поставленим індексом
    outAnResult = tasks[currentIndex];
    // Якщо завдання не завершено, переміщаємо його в кінець черги (якщл не завершено на цьому етапі)
    // У реальному використанні повинні перевірити, чи завдання завершено, чи воно має продовжуватися після свого відрізку часу
    tasks.push_back(tasks[currentIndex]);

    // Видаляємо завершене завдання зі списку завдань
    tasks.erase(tasks.begin() + currentIndex);

    // Переміщаємо індекс вперед
