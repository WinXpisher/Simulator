...
void TaskAnalizer::clearFromCancelled()
{
    anResultClear = anResult;
    for (auto iter = anResultClear.begin(); iter != anResultClear.end();)
    {
        // якщо статус задачі CANCELLED, видаляємо її з вектору
        if ((*iter).taskPtr->status == Task::TaskStatus::CANCELLED)
            iter = anResultClear.erase(iter);
        else
            ++iter;
    }
}