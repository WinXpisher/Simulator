...
private:
    ...
    std::string getLogFileName(int fileNumber)
    {
        ...
        std::string extension = "_" + getDistributionMethodName(selectedDMethod) + ".txt";
    ...
    void initDMethodDirectory(DM::DMethod dMethod)
    {
        if (!logCountForDMethod.count(dMethod))
            logCountForDMethod[dMethod] = fm.getNextFileNumber("log", "_" +
                getDistributionMethodName(selectedDMethod) + ".txt");
    }
public:
